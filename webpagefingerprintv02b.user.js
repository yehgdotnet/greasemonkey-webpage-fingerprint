// ==UserScript==
// @name           WebPageFingerPrintv0.2b
// @namespace      yehg.net
// @description    [Now] Edittable form fields with submit button. Added Ajax Framework Fingerprinting.Likely to be updated over time. [version 0.2]Integrated with JS-file fingerprinting, fuzzing, bruteforcing [Reasons] Sometimes, clicking each in WebDeveloper Toolbar is tedious.I'd like to read a summerized view of current web page first. Here this script comes in. 
...
// @new	       	   Edittable form fields with submit button. Add Ajax Framework Fingerprinting
// @privacy_policy	No data from you is collected, stored or sold to third-parties. I take no interest in you. My time is 100% engaged in learning and keeping up with security trends.
// @license	   GPL v2
// @credits	   Great Thanks for suggestion, encouragement, testing to Mario Heiderich (http://h4k.in), Wordlists taken from  WSfuzzer, Acunetix. Sorry for redundancy. JS Colorizer by Cezary Tomczak, http://code.gosu.pl, Ajax Db List from shreeraj (http://net-square.com)
// @contribute	   Send suggestions, bugs, fixes, help to yehg.org[47]gmail[0]com
// @include        *
// ==/UserScript==
...
var invalidprotocol = /^file|ftp|ftps/;
...
xhr.open("HEAD",yehg_url_scheme+document.domain,false);
xhr.send(null);
...
xhr.send(null);
currentdocument_content = xhr.responseText;
currentdocument_content_length = xhr.responseText.length;
...
unsafeWindow.yehgLoadBruteForceStart = function()
{
	...
	if(s.options[s.selectedIndex].value.indexOf("Select")==-1 && s.options[s.selectedIndex].value.indexOf("HeaderVerbsAbuse")==-1 && s.options[s.selectedIndex].value.indexOf("BackupFiles")==-1)
	{
	    ...
	    unsafeWindow.yehgcurrentjob = "BruteForcing with " + s.options[s.selectedIndex].value;
    }
    else
    {
...
unsafeWindow.yehgLoadBruteForceFile = function()
{
	...
	else
	{
		...
		this.yehgbruteforcestr = document.getElementById(lid).value.toString().split(",");		
		unsafeWindow.yehgBruteForceStart();
		//alert("Cache Content: " + this.yehgbruteforcestr);
	}
...
unsafeWindow.yehgBruteForceStart = function()
{
	...
	if(!document.getElementById(lid))
	{
		unsafeWindow.yehgLoadBruteForceCacheFile(lid,this.yehgbruteforcestr);
		//alert("cached!");
	}	
	
    var bruteforcestr = this.yehgbruteforcestr;    
...
var get_ajax_framework = function(str)
{
    ...
    var Google = /(ga|urchin|__utm)(\.js)/i;
...
for(var s=0;s<=unsafeWindow.document.getElementsByTagName("script").length-1;s++)
{
    ...
    {
        scriptsrc += 'No. ' + s + ' <a  target="_blank"  style="color:yellow!important;text-decoration:underline;" href="javascript:void(0)" onclick="alert(unescape(\'' + escape(unsafeWindow.document.getElementsByTagName("script")[s].innerHTML) + '\'));return false;">Inline JavaScript</a><br>';
    }
...
{
	document.getElementById("yehgfingerprint").style.display="none";document.getElementById("yehgfingerprint2").style.display="";
